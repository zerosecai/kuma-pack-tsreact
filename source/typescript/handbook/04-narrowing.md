<!--
Source: https://raw.githubusercontent.com/microsoft/TypeScript-Website/v2/packages/documentation/copy/en/handbook-v2/Narrowing.md
License: Apache-2.0
...
In addition, classes can [use `this is Type`](https://www.typescriptlang.org/docs/handbook/2/docs/handbook/2/classes.html#this-based-type-guards) to narrow their type.

## Assertion functions

Types can also be narrowed using [Assertion functions](https://www.typescriptlang.org/docs/handbook/2/docs/handbook/release-notes/typescript-3-7.html#assertion-functions).
...
Under [`strictNullChecks`](https://www.typescriptlang.org/docs/handbook/2/tsconfig#strictNullChecks) that gives us an error - which is appropriate since `radius` might not be defined.
...
Additionally, outside of [`strictNullChecks`](https://www.typescriptlang.org/docs/handbook/2/tsconfig#strictNullChecks) we're able to accidentally access any of those fields anyway (since optional properties are just assumed to always be present when reading them).
...
When `radius` was optional, we got an error (with [`strictNullChecks`](https://www.typescriptlang.org/docs/handbook/2/tsconfig#strictNullChecks) enabled) because TypeScript couldn't tell whether the property was present.
...
Both interpretations are correct, but only the union encoding of `Shape` will cause an error regardless of how [`strictNullChecks`](https://www.typescriptlang.org/docs/handbook/2/tsconfig#strictNullChecks) is configured.
