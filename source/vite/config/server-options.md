<!--
Source: https://raw.githubusercontent.com/vitejs/vite/main/docs/config/server-options.md
License: MIT
...
Note that if you are using non-relative [`base`](https://vitejs.dev/config/shared-options.md#base), you must prefix each key with that `base`.
...
In some cases, you might also want to configure the underlying dev server (e.g. to add custom middlewares to the internal [connect](https://github.com/senchalabs/connect) app). In order to do that, you need to write your own [plugin](https://vitejs.dev/guide/using-plugins.html) and use [configureServer](https://vitejs.dev/guide/api-plugin.html#configureserver) function.
...
- **Related:** [Warm Up Frequently Used Files](https://vitejs.dev/guide/performance.html#warm-up-frequently-used-files)
...
If set to `null`, no files will be watched. [`server.watcher`](https://vitejs.dev/guide/api-javascript.html#vitedevserver) will provide a compatible event emitter, but calling `add` or `unwatch` will have no effect.
...
- **Related:** [appType](./shared-options#apptype), [SSR - Setting Up the Dev Server](https://vitejs.dev/guide/ssr#setting-up-the-dev-server)
...
Vite will search for the root of the potential workspace and use it as default. A valid workspace met the following conditions, otherwise will fall back to the [project root](https://vitejs.dev/guide/#index-html-and-project-root).
...
Accepts a path to specify the custom workspace root. Could be a absolute path or a path relative to [project root](https://vitejs.dev/guide/#index-html-and-project-root). For example:
...
This blocklist does not apply to [the public directory](https://vitejs.dev/guide/assets.md#the-public-directory). All files in the public directory are served without any filtering, since they are copied directly to the output directory during build.
